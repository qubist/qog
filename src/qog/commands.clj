...
(defn illegal-move? [con]
	(cond (nil? con) "You can't go that way."
		...
		(and (= location :mine_room_1) (= con :lyre_room)) (do (set-location :mineshaft_bottom) (if (contains? inv :zegg) (do (invrm :zegg) "As you walk into the room, you get hit by something very heavy. When you wake up, you have a egg sized lump on the back of your head and you feel like you are missing something...") "As you walk into the room, you get hit by something very heavy. When you wake up, you have a egg sized lump on the back of your head."))
...
(def commands
	...
	:unlock {
		...
		:fn (fn [p _]
			(if (not (or (= p "elevator") (= p "door") (= p "lock") (= p "trapdoor"))) (println "You can't unlock that.")
							 (cond
